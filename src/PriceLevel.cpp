...
void PriceLevel::addOrder(OrderNode* orderNode) {
  orderNode->next = nullptr; //this is incase that for some reason the orderNode was used somewhere else with a linked list that may carry over, called sanitizeing the pointer values
...
void PriceLevel::removeOrder(OrderNode* orderNode) {
 if (orderNode == nullptr) {
   throw std::invalid_argument("the orderNode is not suppose to be non-existent on the price level");
 } 
 ...
 if (head == tail){
  //in this case that there is only one element in the price level, so it is implied that removing this order would just clear the queue
  head = nullptr;
  tail = nullptr;
  delete orderNode;
  return;
 ...
 if (head == orderNode) {
   head = head->next;
   head->previous = nullptr; //dont forget to make the previous head nullptr after changing to the next in line
   delete orderNode;
   return;
 }
 ...
 if (tail == orderNode) {
   tail = tail->previous;
   tail->next = nullptr; //dont forget to make the previous tail pointer nullptr after changing to previous element
   delete orderNode;
   return;
 }
